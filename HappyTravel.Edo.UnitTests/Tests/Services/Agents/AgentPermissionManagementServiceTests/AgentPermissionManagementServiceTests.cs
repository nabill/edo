    ...
    public class AgentPermissionManagementServiceTests : IDisposable
    {
        ...
        public async Task Set_must_succeed()
        {
            var (isSuccess, _, _, _) = await _agentPermissionManagementService
                .SetInAgencyPermissions(1, InAgencyPermissions.AgentInvitation.ToList(), AgentContextRegular);
        ...
        public async Task Set_relation_not_found_must_fail()
        {
            var (_, isFailure, _, error) = await _agentPermissionManagementService
                .SetInAgencyPermissions(0, InAgencyPermissions.AgentInvitation.ToList(), AgentContextRegular);
