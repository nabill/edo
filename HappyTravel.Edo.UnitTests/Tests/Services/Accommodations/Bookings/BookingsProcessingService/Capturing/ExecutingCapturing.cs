...
namespace HappyTravel.Edo.UnitTests.Tests.Services.Accommodations.Bookings.BookingsProcessingService.Capturing
{
    public class ExecutingCapturing
    {
        ...
        private Api.Services.Accommodations.Bookings.BookingsProcessingService CreateProcessingService(IBookingPaymentService bookingPaymentService)
        {
            var context = MockEdoContextFactory.Create();
            context.Setup(c => c.Bookings)
                .Returns(DbSetMockProvider.GetDbSetMock(Bookings));
            
            var service = new Api.Services.Accommodations.Bookings.BookingsProcessingService(bookingPaymentService,
